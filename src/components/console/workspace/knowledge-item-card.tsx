...
interface DraggableKnowledgeItemCardProps {
  item: KnowledgeItem
}
...
export function DraggableKnowledgeItemCard({ item }: DraggableKnowledgeItemCardProps) {
  ...
  const handleDragStart = (e: React.DragEvent) => {
    ...
    e.dataTransfer.setData(
      ...
      JSON.stringify({
        type: "link",
        url: item.sourceUrl,
        label: item.summary || item.sourceUrl,
        itemId: item.id,
      })
    )
  }
  ...
  return (
    <>
      <Card
        className={cn(
          "group hover:shadow-lg hover:border-primary/20 transition-all duration-300 overflow-hidden relative z-0 hover:z-20 ml-2 mt-3 hover:ring-4 hover:ring-primary/10 cursor-grab active:cursor-grabbing",
